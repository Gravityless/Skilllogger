...
│   ├── common/                    # 服务端 fixture / 双端 client runner
│   ├── test_client_bash.py        # bash client C1–C7
│   ├── test_client_ps.py          # pwsh client C1–C7
...
> **重要**：脚本被设计为**完全静默 + 永远 exit 0**，不会污染大模型上下文，也绝不会中断 skill 主流程。失败会被默默吞掉并把事件落到本地队列。
...
- **并发**：用原子重命名（`rename` / `Move-Item`）认领待发送批次和孤儿文件，配合 append-only 写入主队列，避免多个 skill 并发时**重复上报**或**撕裂行**
...
`bash` 与 `pwsh` 客户端骨架对称，都把整段逻辑包在 `try` / 子 shell 内并强制 `exit 0`，**任何异常都不会传出**。核心是一个 4 步状态机：
...
- 两端 client 七种场景（在线、离线入队、离线后补传、孤儿 sending 文件回收、新 sending 文件不被误回收、缺参数静默退出、损坏 JSON 行跳过）
